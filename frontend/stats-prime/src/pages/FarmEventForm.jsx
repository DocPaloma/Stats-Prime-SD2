...
import { createFarmEvent, getGames } from "../api/farmEvents";
...
function FarmEventRegister() {
    ...
    useEffect(() => {
        ...
        axiosClient
            .get(`/games/${selectedGame}/sources/${form.source}/rewards/`)
            .then((res) => {
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              setForm((prev) => ({ ...prev, farm_type: e.target.value }))
            }
              ...
              <tbody>
                {form.drops.map((drop, index) => (
                  <tr key={drop.id} className="border-b border-slate-700">
                    <td className="p-2">{drop.name}</td>
                    <td className="p-2">{drop.rarity_display}</td>
